        ...
        <div class="table-responsive">
          <table id="table-1" class="table table-bordered text-nowrap border-bottom w-100">
            <thead>
              <tr>
                <th width="1%">No</th>
                <th>Gambar</th><th>Kode</th><th>Nama</th>
                <th>Jenis</th><th>Satuan</th><th>Merk</th>
...
let table=$('#table-1').DataTable({
  processing:true, serverSide:true, stateSave:true, scrollX:true,
...
/* ================================================================= */
function resetRows(){ $('#barang-rows').empty(); addRow(); setLoading(false); }
function addRow(){
...
function checkForm(){
  ...
  if(!ok){ setLoading(false); swal('Isian wajib masih kosong','','warning'); return; }
  submitForm();
}
...
function submitForm(){
  ...
  $.post({url:"{{ route('barang.store') }}",data:fd,
          processData:false,contentType:false})
   .done(()=>{ $('#modaldemo8').modal('hide');
               swal('Berhasil ditambah','','success');
               table.ajax.reload(null,false); resetRows(); })
   .fail(x=>{ swal('Error',x.responseText,'error'); setLoading(false); });
}
...
function VerifyFileNameAndFileSize(i){
  ...
  if(!['png','jpg','jpeg','svg'].includes(ext)){ swal('Format salah','','warning'); i.value=''; return; }
  if(f.size>3*1024*1024){ swal('Max 3 MB','','warning'); i.value=''; return; }
...
$('#formEdit').on('submit',function(e){
  e.preventDefault();
  $('#btnUpdateLoad').removeClass('d-none'); $('#btnUpdate').addClass('d-none');
  ...
  .done(()=>{ $('#Umodaldemo8').modal('hide');
              swal('Berhasil diperbarui','','success');
              table.ajax.reload(null,false); })
  .fail(x=>{ swal('Error',x.responseText,'error'); })
  .always(()=>{ $('#btnUpdateLoad').addClass('d-none');
...
$('#btnConfirmDelete').on('click', function () {
    ...
    $.ajax({
        ...
        data: { _token: "{{ csrf_token() }}" },   // wajib kalau pakai Laravel
