...
class UserController extends Controller
{
    ...
    public function updateProfile(Request $request, UserModel $user)
    {
        ...
        if ($request->hasFile('photoU')) {
            ...
            // Simpan file langsung ke public/storage/users
            $request->file('photoU')->move(public_path('storage/users'), $imageName);

            //delete old image if not default
            if ($user->user_foto != 'undraw_profile.svg' && $user->user_foto != '') {
                // Hapus dari public/storage
                if (file_exists(public_path('storage/users/' . $user->user_foto))) {
                    unlink(public_path('storage/users/' . $user->user_foto));
                }
            }
